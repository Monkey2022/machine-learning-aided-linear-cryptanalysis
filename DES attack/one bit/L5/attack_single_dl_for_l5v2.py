...
#only for 5-round linear expression of des
...
def obtain_subkey_use_L5(p,c,sample_pair,pc_pair):
    ...
    sample=Ph[:,31-15]^Pl[:,31-7]^Pl[:,31-18]^Pl[:,31-24]^Pl[:,31-27]^Pl[:,31-28]^Pl[:,31-29]^Pl[:,31-30]^Pl[:,31-31]^Ch[:,31-15]^Cl[:,31-7]^Cl[:,31-18]^Cl[:,31-24]^Cl[:,31-27]^Cl[:,31-28]^Cl[:,31-29]^Cl[:,31-30]^Cl[:,31-31]
...
def attack_5_round_use_L5(pc_pair,sample_pair):
    
    

    keys = np.frombuffer(urandom(64), dtype=np.uint8).reshape(-1,64)
    keys = keys & 1
    subkey=ciph.expand_key(keys,get_Round())#攻击的轮数比等式的轮数多1轮
    ...
    real_key=subkey[0][:,47-42]^subkey[0][:,47-43]^subkey[0][:,47-45]^subkey[0][:,47-46]^subkey[1][:,47-22]^subkey[3][:,47-22]^subkey[4][:,47-42]^subkey[4][:,47-43]^subkey[4][:,47-45]^subkey[4][:,47-46]
    real_key=real_key[0]
    
    
    guess_key=obtain_subkey_use_L5(plain,cipher,sample_pair,pc_pair)
...
Pc_pair=[16,20,22,24,26,28,30,32,34,52,60,70,80,90,100,110,120,130,140]
Sample_pair=[1]
...
for pc_pair in Pc_pair:
    for sample_pair in Sample_pair:
        ...
        for i in range(num):   
            real_key,guess_key=attack_5_round_use_L5(pc_pair,int(num_pair/pc_pair))
